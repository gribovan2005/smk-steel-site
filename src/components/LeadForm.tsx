...
const leadSchema = z.object({
    ...
    phone: z.string().trim().min(7, "Укажите телефон").max(30, "Слишком длинный номер"),
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" encType="multipart/form-data">
            ...
            hover:file:bg-black hover:file:text-white hover:file:border-white"
                />
                {files.length > 0 && <p className="mt-1 text-xs text-gray-400">Выбрано файлов: {files.length}</p>}
            </div>

            <button type="submit" disabled={isSubmitting} className="btn w-full sm:w-auto disabled:opacity-60 text-sm">
                {isSubmitting ? "Отправка..." : "Отправить заявку"}
            </button>

            {status === "ok" && <p className="text-green-400 text-sm">Заявка отправлена. Мы свяжемся с вами.</p>}
            {status === "error" && <p className="text-red-400 text-sm">Ошибка отправки. Попробуйте позже.</p>}
        </form>
    );
}
