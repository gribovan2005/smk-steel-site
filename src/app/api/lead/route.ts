                ...
                if (!url) {
                    try {
                        const uploaded = await put(
                            `leads/${Date.now()}-${encodeURIComponent(f.name || `file-${idx + 1}`)}`,
                            buffer,
            ...
            await Promise.all([
                sendTelegram(
                    text +
                        (downloadUrls[0]
                            ? "\n\nСсылка:\n" +
                              `${(process.env.NEXT_PUBLIC_SITE_URL || "") + "/api/file?url=" + encodeURIComponent(downloadUrls[0]) + "&name=" + encodeURIComponent(filenames[0] || "file")}`
                            : "")
                ).catch(() => {}),
                appendLeadToSheet([
